...
public class CommentService {
    ...
    // 👆 changed from <String, CommentEvent> → <String, Object>
    // because we want to handle multiple event types with one converter
    ...
    public CommentResponseDto addCommentToAnswer(CommentRequestDto commentRequestDto, String username) {
        ...
        try {
            ...
            kafkaTemplate.send(
                    MessageBuilder
                            .withPayload(event)
                            .setHeader(KafkaHeaders.TOPIC, KafkaConfig.NOTIFICATIONS_TOPIC)
                            .setHeader("__TypeId__", "COMMENT_EVENT") // 👈 the key fix
                            .build()
            );
        } catch (Exception e) {
        ...
        try {
            ...
            kafkaTemplate.send(
                    MessageBuilder
                            .withPayload(event)
                            .setHeader(KafkaHeaders.TOPIC, KafkaConfig.NOTIFICATIONS_TOPIC)
                            .setHeader("__TypeId__", "COMMENT_EVENT")
                            .build()
            );
